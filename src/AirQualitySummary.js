...
const AirQualitySummary = () => {
    ...
    const { cities, city, country, convertCountry, address, datasets, currentData, dateRange, monthNames, downloadData, airQualityData, airQuality } = useContext(AppContext);
    ...
    useEffect(() => {
        ...
        airQualityData.forEach((d) => {
            if(d.year >= dateRange[0] && d.year <= dateRange[1]) {
                chartData.push({year: d.year, value: parseFloat(d.Aerosol_Optical_Depth_Land_Ocean_Mean_Mean), trendline: 0});
            }
        });
